...
class Window(QWidget):
    ...
    def init_ui(self):
        ...
        self.port_list.setMinimumSize(500,380)
    ...
    def start_port_scanner(self):
        self.port_scanner = PortScanner("ahmetcankaraagacli.com",self.port_list)
        self.port_scanner.progressbar_changer.connect(self.update_progress_bar_port)
        self.port_scanner.start()

    def start_sub_scanner(self):
        self.sub_scanner = SubdomainScanner("instra.com",self.sub_list,self.subscan_bar)
        #self.sub_scanner.sub_bar_changer.connect(self.update_progress_bar_sub)
        self.sub_scanner.start()



    def start_dir_scanner(self):
        self.dir_scanner = DirScanner("https://bekchy.com",self.dir_list,self.dirscan_bar)
        #self.dir_scanner.progressbar_changer3.connect(self.update_progress_bar_dir)
        self.dir_scanner.start()

    def start_header_analysis(self):
        header_analy = HeaderAnalysis("https://bekchy.com",self.header_list)

    def test_progres_bar(self,text,bar_counter):
        self.port_bar.setFormat( text );
        self.port_bar.setValue((100*bar_counter)/(65536))
